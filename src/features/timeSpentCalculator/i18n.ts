import { injectTranslation } from '@/shared/lib/i18n';

const translations = {
	ru: {
		workTimeInput: {
			title: 'Ваш типичный рабочий день',
			startTime: 'Начало рабочего дня',
			endTime: 'Конец рабочего дня',
			includeLunch: 'Обед',
			lunchStart: 'Начало обеда',
			lunchEnd: 'Конец обеда',
		},
		salaryInput: {
			title: 'Зарплата на руки',
			rubles_a_month_one: 'рубль в месяц',
			rubles_a_month_few: 'рубля в месяц',
			rubles_a_month_many: 'рублей в месяц',
			description: 'Если у вас нестабильный оклад, укажите среднемесячную зарплату за последние полгода.',
		},
		periods: {
			label: 'Период',
			day: 'в день',
			month: 'в месяц',
			year: 'в год',
		},
		addGroup: {
			button: 'Добавить группу',
			title: 'Новая группа расходов',
			name: 'Название группы',
			description: 'Описание (необязательно)',
			color: 'Цвет группы',
			add: 'Добавить',
		},
		deleteGroup: {
			button: 'Удалить группу',
			confirmTitle: 'Удалить группу?',
			confirmMessage: 'Вы уверены, что хотите удалить группу "{{name}}"? Это действие нельзя отменить.',
			confirm: 'Удалить',
			cancel: 'Отмена',
		},
		progress: {
			title: 'Ваш рабочий день',
			breakdown: 'Разбивка по группам',
			summary: 'Итого',
			noExpenses: 'Добавьте расходы, чтобы увидеть результаты',
			totalHours: 'Всего часов: {{hours}}',
			salary: 'Зарплата: {{amount}} ₽',
			monthlyExpenses: 'Месячные расходы: {{amount}} ₽',
			expensesPercentage: 'Расходы составляют {{percentage}}% от зарплаты',
			workTimeForExpenses: 'Время работы на расходы: {{hours}}ч',
			savings: 'Свободные средства: {{amount}} ₽ ({{hours}}ч работы)',
			ofSalary: 'от зарплаты',
			freeFunds: 'Свободные средства',
			hoursShort: 'ч',
			minutesShort: 'мин',
			freeTime: 'Свободное время',
			freeTimePerDay: 'Свободное время в день',
			deficitPerDay: 'Не хватает в день',
			expensesOfSalary: '{{spent}} ₽ из {{salary}} ₽ в месяц',
			savingsSegment: 'Накопления',
			overflow_one: '+{{count}} час',
			overflow_few: '+{{count}} часа',
			overflow_many: '+{{count}} часов',
		},
		common: {
			cancel: 'Отмена',
		},
		main: {
			title: 'Ради чего вы работаете?',
			expensesByGroups: 'Расходы по группам',
			addExpense: 'Добавить расход',
			expenseName: 'Название',
			expenseAmount: 'Сумма',
		},
		groups: {
			apartment: 'Квартира',
			transport: 'Транспорт',
			food: 'Еда',
			subscriptions: 'Подписки',
			entertainment: 'Развлечения',
			health: 'Здоровье',
			// Названия статей расходов
			rent: 'Съем квартиры',
			utilities: 'Квартплата',
			cleaning: 'Клининг',
			gasoline: 'Бензин',
			taxi: 'Такси',
			products: 'Продукты',
			coffee: 'Кофе/кафе',
			netflix: 'Netflix',
			spotify: 'Spotify',
			gym: 'Фитнес-клуб',
			cinema: 'Кино/театр',
			games: 'Игры',
			dentistry: 'Стоматология',
			medicine: 'Лекарства',
		},
	},
	en: {
		workTimeInput: {
			title: 'Your typical work day',
			startTime: 'Start time',
			endTime: 'End time',
			includeLunch: 'Include lunch time',
			lunchStart: 'Lunch start',
			lunchEnd: 'Lunch end',
		},
		salaryInput: {
			title: 'Salary after tax',
			rubles_a_month_one: 'ruble in a month',
			rubles_a_month_other: 'rubles in a month',
			description: 'If your salary is not stable, please enter your average salary for the last 6 months.',
		},
		periods: {
			label: 'Period',
			day: 'a day',
			month: 'a month',
			year: 'a year',
		},
		addGroup: {
			button: 'Add group',
			title: 'New expense group',
			name: 'Group name',
			description: 'Description (optional)',
			color: 'Group color',
			add: 'Add',
		},
		deleteGroup: {
			button: 'Delete group',
			confirmTitle: 'Delete group?',
			confirmMessage: 'Are you sure you want to delete the group "{{name}}"? This action cannot be undone.',
			confirm: 'Delete',
			cancel: 'Cancel',
		},
		progress: {
			title: 'Your work day',
			breakdown: 'Breakdown by groups',
			summary: 'Summary',
			noExpenses: 'Add expenses to see results',
			totalHours: 'Total hours: {{hours}}',
			salary: 'Salary: {{amount}} ₽',
			monthlyExpenses: 'Monthly expenses: {{amount}} ₽',
			expensesPercentage: 'Expenses are {{percentage}}% of salary',
			workTimeForExpenses: 'Work time for expenses: {{hours}}h',
			savings: 'Free funds: {{amount}} ₽ ({{hours}}h of work)',
			ofSalary: 'of salary',
			freeFunds: 'Free funds',
			hoursShort: 'h',
			minutesShort: 'min',
			freeTime: 'Free time',
			freeTimePerDay: 'Free time per day',
			deficitPerDay: 'Short by, per day',
			expensesOfSalary: '{{spent}} of {{salary}} ₽ per month',
			savingsSegment: 'Savings',
			overflow_one: '+{{count}} hour',
			overflow_other: '+{{count}} hours',
		},
		common: {
			cancel: 'Cancel',
		},
		main: {
			title: 'What do you work for?',
			expensesByGroups: 'Expenses by groups',
			addExpense: 'Add expense',
			expenseName: 'Name',
			expenseAmount: 'Amount',
		},
		groups: {
			apartment: 'Apartment',
			transport: 'Transport',
			food: 'Food',
			subscriptions: 'Subscriptions',
			entertainment: 'Entertainment',
			health: 'Health',
			// Названия статей расходов
			rent: 'Rent',
			utilities: 'Utilities',
			cleaning: 'Cleaning',
			gasoline: 'Gasoline',
			taxi: 'Taxi',
			products: 'Products',
			coffee: 'Coffee/Cafe',
			netflix: 'Netflix',
			spotify: 'Spotify',
			gym: 'Gym',
			cinema: 'Cinema/Theater',
			games: 'Games',
			dentistry: 'Dentistry',
			medicine: 'Medicine',
		},
	},
};

export const TRANS_NS = 'timeSpentCalculator';

injectTranslation(TRANS_NS, translations);
